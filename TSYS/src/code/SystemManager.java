...
public class SystemManager {
	...
	private String cfilename = "C:\\pleiades\\workspace\\TSYS\\src\\database\\client.csv";
	...
	public static void main( String[] args ) {
		//try,catch文
		try {									//tryの中身を実行して、その中で例外が発生した場合にcatchに移る。
			SystemManager manager = new SystemManager();

			manager.load();						//ファイルをロード
			manager.run();                 		//メインルーチン
			manager.save();						//ファイルをセーブ

		} catch ( Exception e ) {				//例外処理　
			e.printStackTrace();				//エラーの原因出力
			System.exit( 0 );
		}
	}
	...
	public void statusSetting() {
		...
		// システム起動時の，機能選択の状態
		sts1 = new ConsoleStatus(				//画面のインスタンスを作る。
		...
		//メイン画面の分岐 sts1
		sts1.setNextStatus( "S", sts2 );		//入力されたものとそれに対応した処理のクラスをMAPで紐づける
	...
	public void run() throws Exception {
		...
		while( !sts.getIsEndStatus() ) {				//メインルーチンの繰り返し
			// 最初に出力するメッセージ
	...
	public void load() throws Exception {
		// 各CSVファイルからレコードを読み取る
		FileLoader pload = new FileLoader( pfilename );	//３つのインスタンスで起動。
	 ...
	 * @throws Exception
	 */
	public void save() throws Exception {					//Xが押されると、保存するこの処理に移る。
